...
export const TaskListRow = memo(function TaskListRow({ task, onClick }: { task: ProjectTaskView; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="grid h-12 w-full grid-cols-[1fr_140px_110px_80px] items-center gap-3 px-4 text-left hover:bg-[#F5EFE6]"
    >
      <div className="flex min-w-0 items-center gap-3">
        <span className="size-1.5 rounded-full" style={{ background: PRIORITY_DOT[task.priority] }} />
        <span className={cn("truncate text-sm font-medium", done && "line-through text-[#9C8573]")}>{task.title}</span>
      </div>
...
export function TaskListHeader() {
  return (
    <div className="grid h-10 grid-cols-[1fr_140px_110px_80px] items-center gap-3 border-b bg-[#F5EFE6] px-4 text-[11px] font-medium text-[#9C8573] uppercase">
      <span>Task</span>
      <span>Assignee</span>
      <span>Status</span>
